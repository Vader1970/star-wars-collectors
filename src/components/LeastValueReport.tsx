...
interface LeastValueReportProps {
  open: boolean;
  onClose: () => void;
}
...
const LeastValueReport: React.FC<LeastValueReportProps> = ({ open, onClose }) => {
  const { items } = useCollection();
  const router = useRouter();

  // Get items with valuations, sorted by value ascending (lowest first)
  const leastValueItems = [...items]
  ...
  return (
    <Dialog open={open} onOpenChange={(state) => !state && onClose()}>
      <DialogContent className='max-w-4xl bg-white border border-slate-300'>
        <DialogHeader>
          <DialogDescription className='sr-only'>Least Valuable Items</DialogDescription>
          <DialogTitle className='text-2xl font-bold flex flex-row items-center justify-between'>
            <span>
              ...
              <span className='font-bold text-indigo-700 ml-2 text-lg'>{leastValueItems.length}</span>
            </span>
            <Button
              variant='ghost'
              size='icon'
...
export default LeastValueReport;
