...
import { BarChart3, Star, List, Heart, Coins, TrendingUp } from "lucide-react";
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="right" className="max-w-md w-full bg-background border-l border-slate-700 px-0 flex flex-col">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4">
              {categories.map((cat) => (
                ...
                >
                  ...
                  <span className="font-semibold text-lg text-white text-center group-hover:text-blue-300">{cat.label}</span>
                </button>
              ))}
            </div>
          </ScrollArea>
      ...
      <Top10Report open={top10ReportOpen} onClose={() => { setTop10ReportOpen(false); onOpenChange(false); }} />
      <WishlistReport open={wishlistReportOpen} onClose={() => { setWishlistReportOpen(false); onOpenChange(false); }} />
      <CategoryValuationReport open={groupReportOpen} onClose={() => { setGroupReportOpen(false); onOpenChange(false); }} />
      <LeastValueReport open={leastValueReportOpen} onClose={() => { setLeastValueReportOpen(false); onOpenChange(false); }} />
      <ValuationReport open={valuationReportOpen} onClose={() => { setValuationReportOpen(false); onOpenChange(false); }} />
    </>
  );
};
