...
const ItemCard = ({ item, onEdit, onDelete, onViewDetails, showActions = true }: ItemCardProps) => {
  ...
  return (
    <Card className={`bg-white border border-gray-200 hover:border-gray-300 transition-all duration-200 group cursor-pointer flex flex-col ${MIN_CARD_HEIGHT}`}>
      <CardContent className="p-0 flex flex-col flex-1 h-full">
          ...
          {showActions && (
            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <Button
                ...
                className="h-8 w-8 p-0 bg-white/90 hover:bg-white border-red-200 text-red-600 hover:bg-red-50"
                aria-label="Delete"
              >
        ...
        <div className="p-4 flex flex-col flex-1 justify-between min-h-[180px]">
          <div>
            <h3 className="font-semibold text-gray-800 mb-3 text-lg line-clamp-2 min-h-[2.4em]">
              {item.name}
            </h3>
