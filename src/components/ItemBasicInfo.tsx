...
const ItemBasicInfo: React.FC<ItemBasicInfoProps> = ({
  item,
  group,
  category
}) => {
  const buildEbaySearchUrl = (group: string, name: string, year?: number) => {
    const yearString = year ? year.toString() : '';
    const query = `${group} ${name} ${yearString}`.trim().replace(/\s+/g, '+');
    return `https://www.ebay.com/sch/i.html?_nkw=${query}&_sacat=0&_trksid=p4432023.m570.l1313`;
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="pt-4 border-t border-gray-200">
          <a
            href={buildEbaySearchUrl(group, item.name, item.yearManufactured)}
            target="_blank"
