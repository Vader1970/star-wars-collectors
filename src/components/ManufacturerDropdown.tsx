...
export const ManufacturerDropdown: React.FC<ManufacturerDropdownProps> = ({
  ...
  return (
    <Select value={value} onValueChange={onChange}>
      <SelectTrigger id='manufacturer' name='manufacturer' className='bg-white border-gray-300 text-black'>
      ...
      <SelectContent className='bg-white max-h-60 overflow-y-auto z-[60]'>
        {manufacturers.map((manufacturer, idx) => (
          <div key={manufacturer} className='flex items-center w-full group'>
            <SelectItem id={`manufacturer-${manufacturer}`} value={manufacturer} className='flex-1'>
              {manufacturer}
            </SelectItem>
            <Button
              type='button'
